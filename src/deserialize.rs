...
pub fn deserialize(args: Cli) -> ConfigToml {
    ...
    let mut config_toml: ConfigToml = toml::from_str(&toml_str).expect("Failed to deserialize Cargo.toml");

    let binding = PathBuf::from(&args.path);
    let config_dir = binding.parent().expect("Failed to get parent directory of config file");
